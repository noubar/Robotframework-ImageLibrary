...
class ImageHorizonLibrary(DynamicCore):
    ...
    *Key features*: 
    ...
    def __init__(self, reference_folder=None, screenshot_folder=None,
                 ...
                 edge_sigma=2.0, edge_low_threshold=0.1, edge_high_threshold=0.3, recognition_timeout=0):
        ...
        """
        self.orechesterer = Orchesterer()
        self.defaults = self.orechesterer.Defaults(reference_folder, keyword_on_failure)
        self.screenshots = self.orechesterer.Screenshots(screenshot_folder,1)
        self.recognizer = self.orechesterer.Recognitions(confidence, strategy, edge_sigma,
                                                        edge_low_threshold, edge_high_threshold,
                                                        recognition_timeout)
        libraries = [KeyboardKeywords(),
                    MouseKeywords(),
                    ScreenshotKeywords(self.screenshots),
                    OperatingSystemKeywords(self.defaults),
                    RecognizeImagesKeywords(self.defaults, self.recognizer),
                    OrganizeKeywords(self.defaults, self.recognizer)
                    ]
    ...
    def _run_on_failure(self):
        ...
        try:
            ...
            LOGGER.debug(e)
            LOGGER.warn('Failed to run keyword_on_failure. '
